...
import { PRODUCT_BY_ID_QUERY, RELATED_PRODUCTS_QUERY } from "../lib/queries"
...
export default function ProductDetail() {
  ...
  if (!product) return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-4">
      <p className="font-sans text-gray-400 text-sm">Product not found.</p>
      <Link to="/shop" className="font-heading text-xs tracking-widest uppercase underline text-gray-500">
        Back to Shop
      </Link>
    </div>
  )
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <div className="px-4 md:px-12 py-4 flex items-center gap-2 text-xs text-gray-300 border-b border-gray-50">
        <Link to="/" className="hover:text-gray-600 transition-colors font-sans">Home</Link>
        <span>/</span>
        <Link to="/shop" className="hover:text-gray-600 transition-colors font-sans">Shop</Link>
        <span>/</span>
        <span className="text-gray-500 font-sans">{product.name}</span>
      </div>
      ...
      <div className="px-4 md:px-12 py-8 md:py-12 grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 max-w-6xl mx-auto items-start">
        ...
        <div className="flex flex-col">
          <div className="flex items-center gap-3 mb-3">
            <span className="font-sans text-[10px] tracking-widest uppercase text-gray-300 bg-gray-50 px-3 py-1 rounded-full">
              {product.category}
            </span>
            <span className="font-sans text-[10px] tracking-widest uppercase text-gray-300">
              {product.age}
            </span>
          </div>
          ...
          <div className="flex items-baseline gap-3 mb-6">
            <span className="font-heading text-2xl font-medium text-gray-900">
              {formatPrice(product.price)}
            </span>
            {product.oldPrice && (
              <>
                <span className="font-sans text-base text-gray-300 line-through">
                  {formatPrice(product.oldPrice)}
                </span>
                <span className="font-badge text-xs bg-orange-50 text-orange-600 px-2 py-1 rounded-full">
          ...
          <p className="font-sans text-sm text-gray-400 leading-relaxed mb-6 font-light">
            Crafted with premium soft fabric, perfect for active little ones.
            Breathable, easy to wash, and designed to grow with your child.
            Made with love for ages {product.age}.
          </p>
          ...
          <div className="grid grid-cols-3 gap-2 mb-6">
            {[
              ...
              <div key={item.label} className="bg-gray-50 rounded-xl p-3 text-center">
                ...
                <p className="font-heading text-[10px] font-medium text-gray-700">{item.label}</p>
                <p className="font-sans text-[9px] text-gray-400">{item.sub}</p>
              </div>
            ))}
          </div>
          ...
          <div className="border-t border-gray-100 pt-4">
            <div className="flex items-center justify-between py-3">
              <p className="font-sans text-xs tracking-widest uppercase text-gray-500">Product Details</p>
              <svg className="w-4 h-4 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 9l-7 7-7-7" />
              </svg>
            </div>
            <div className="font-sans text-xs text-gray-400 leading-relaxed pb-3 font-light">
              <p>• Material: 100% Premium Cotton</p>
              <p>• Machine washable at 30°C</p>
              <p>• Available in {product.colors?.length || 1} color{product.colors?.length > 1 ? "s" : ""}</p>
      ...
      {related.length > 0 && (
        <section className="px-4 md:px-12 py-10 md:py-14 border-t border-gray-50 max-w-6xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link to="/shop" className="font-sans text-xs tracking-widest uppercase text-gray-400 hover:text-gray-900 transition-colors">
              View All →
            </Link>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
            {related.map((p) => (
              ...
              >
                ...
                <div className="p-3">
                  <p className="font-sans text-[10px] text-gray-300 uppercase tracking-widest mb-1">{p.age}</p>
                  <p className="font-heading text-sm font-medium text-gray-900 mb-1">{p.name}</p>
                  <p className="font-heading text-sm font-medium">{formatPrice(p.price)}</p>
                </div>
              </Link>
            ))}
          </div>
